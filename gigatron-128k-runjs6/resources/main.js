...
$(function() {
    ...
    function adjustWindowSize() {
        if (typeof Neutralino !== 'undefined') {
            ...
            let canvasWidth = vgaCanvas.width();
            let canvasHeight = vgaCanvas.height();
            
            // 计算需要的窗口大小（考虑边框、标题栏等）
            let windowWidth = canvasWidth + 40; // 左右边距
            let windowHeight = canvasHeight + 200; // 上下边距和控制面板空间
            ...
            Neutralino.window.getSize().then(size => {
                ...
                if (windowWidth > size.width || windowHeight > size.height) {
                    Neutralino.window.setSize({
                        width: Math.max(windowWidth, size.width),
                        height: Math.max(windowHeight, size.height)
                    });
                }
            });
        }
